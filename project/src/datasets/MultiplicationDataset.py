...
class MultiplicationDataset(Dataset):
    ...
    def __init__(self, a_max=999, b_max=99, eos_token="<EOS>", input_len=7, output_len=5):
        ...
        self.pairs = [(a, b) for a in range(1, a_max + 1) for b in range(1, b_max + 1)]
    ...
    def __getitem__(self, idx: int) -> Any:
        ...
        return {
            "input": [self.token_idx[token] for token in input_tokens],
            "output": [self.token_idx[token] for token in output_tokens],
        }
...
d1 = MultiplicationDataset(a_max=2, b_max=2, eos_token="<EOS>")
...
d2 = MultiplicationDataset(a_max=99, b_max=99, eos_token="<EOS>")
